...
public class NN2ProcessByTrend {
    ...
    public void processNN40InputNeuralNetTrend(ServiceAFweb serviceAFWeb) {
        ////////////////////////////////////////////
...
//        serviceAFWeb.forceNNReadFileflag = false; // should be true to get it from file instead from db
        ...
        if (initTrainNeuralNet == true) {
            ...
            double errorNN = CKey.NN40_ERROR_THRESHOLD;
            ...
            if (flagInit == true) {
                ...
                if (afNeuralNet == null) {
                    ...
                    String weightSt = (CKey.NN40_WEIGHT_0);
...
//                    serviceAFWeb.getStockImp().setCreateNeuralNetObjSameObj1(BPname, refname, weightSt);
                    ...
                    logger.info(">>> NeuralNetProcessTesting " + BPname + " using NN40_WEIGHT_0");
                } else {
...
//            ServiceAFweb.writeArrayNeuralNet.addAll(writeArray);
    ...
    public boolean NeuralNetAllStockNN40CreatJava(ServiceAFweb serviceAFWeb, String nnName) {
        ...
        try {
            ...
            String fileN = ServiceAFweb.FileLocalDebugPath + "nn40AllData.java";
            FileUtil.FileWriteText(fileN, msgWrite);
            return true;
        } catch (Exception ex) {
        }
    ...
    private ArrayList UpdateStockNN40processNameArray(ServiceAFweb serviceAFWeb, AccountObj accountObj) {
...
//        }
        ...
        if (stockNameArray != null) {
            ...
            for (int i = 0; i < stockNameArray.size(); i++) {
                ...
                String symTR = sym + "#" + ConstantKey.INT_TR_NN40;
    ...
    public void ProcessTrainNeuralNetNN2tByTrend(ServiceAFweb serviceAFWeb) {
        ...
        logger.info("ProcessTrainNeuralNeNN2tByTrend " + printName);

        String LockName = null;
        Calendar dateNow = TimeConvertion.getCurrentCalendar();
        long lockDateValue = dateNow.getTimeInMillis();

        LockName = "NN40_" + ServiceAFweb.getServerObj().getServerName();
...
//                    symbol = "BABA";
                    ...
                    this.PTrainNN40NeuralNetByTrend(serviceAFWeb, symbol, TR_NN, stockNNprocessNameArray);
...
//                            }
//                        } catch (Exception ex) {
//                            logger.info("> ProcessTrainNeuralNeNN2tByTrend Exception" + ex.getMessage());
//                        }
...
//            logger.info("ProcessTrainNeuralNetByTrend " + LockName + " unlock LockName");
        ...
        logger.info("> ProcessTrainNeuralNeNN2tByTrend ... done");
    }

    public void PTrainNN40NeuralNetByTrend(ServiceAFweb serviceAFWeb, String symbol, int TR_NN, ArrayList stockNNprocessNameArray) {
        String LockStock = "NN40_TR_" + symbol; // + "_" + trNN;
        LockStock = LockStock.toUpperCase();
...
//                                    }
                ...
                logger.info("> PTrainNN40NeuralNetByTrend Exception" + ex.getMessage());
            }
            serviceAFWeb.removeNameLock(LockStock, ConstantKey.NN_TR_LOCKTYPE);
//                        logger.info("ProcessTrainNeuralNetByTrend " + LockStock + " unLock LockStock ");
        }
...
//        logger.info("> trainingNN " + symbol);
        ...
        if (tr == ConstantKey.INT_TR_NN40) {
            //StockArray assume recent date to old data  
            //StockArray assume recent date to old data              
            //trainingNN1dataMACD will return oldest first to new date
            //trainingNN1dataMACD will return oldest first to new date            
            ProcessNN00 nn00 = new ProcessNN00();
            inputList = nn00.trainingNN00_dataEMA0(serviceAFWeb, symbol, StockArray, offset, CKey.MONTH_SIZE + 10); // 14
