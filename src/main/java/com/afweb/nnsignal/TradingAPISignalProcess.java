...
public class TradingAPISignalProcess {
    ...
    private ArrayList UpdateStockSignalNameArray(ServiceAFweb serviceAFWeb) {
        ...
        for (int i = 0; i < custList.size(); i++) {
            ...
            AccountObj accountObj = serviceAFWeb.AccGetAccountByType(cust.getUsername(), null, AccountObj.INT_TRADING_ACCOUNT);
        ...
        if (lockReturn > 0) {
            ...
            for (int i = 0; i < 30; i++) {
                ...
                try {
                    ...
                    AccountObj accountObj = serviceAFWeb.AccGetAccountByAccountID(accId);
...
//                    logger.info("ProcessAdminSignalTrading " + LockStock + " LockStock " + lockReturnStock);
                    if (lockReturnStock > 0) {
                        ...
                        serviceAFWeb.SysLockRemoveLockName(LockStock, ConstantKey.ADMIN_SIGNAL_LOCKTYPE);
//                        logger.info("ProcessAPISignalTrading " + LockStock + " unLock LockStock ");
            ...
            serviceAFWeb.SysLockRemoveLockName(LockName, ConstantKey.ADMIN_SIGNAL_LOCKTYPE);
