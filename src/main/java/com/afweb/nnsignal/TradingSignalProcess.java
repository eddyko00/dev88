...
//        symbol = "DIA";
        ...
        ArrayList tradingRuleList = serviceAFWeb.AccGetAccountStockTRListByAccIdSym(accountObj.getId(), symbol);
...
//            }
            ...
            float perf = serviceAFWeb.AccGetAccountStockBalance(trObj);
            if (perf == -9999) {
                ;
            } else {
                trObj.setPerf(perf);
...
//                    logger.info("> upateAdminTransaction INITIAL " + stock.getSymbol() + " " + trObj.getTrname());
                    serviceAFWeb.AccClearAccountStockTranByAccountID(accountObj, stock.getId(), trObj.getTrname());
                    ...
                    ArrayList<TransationOrderObj> currTranOrderList = serviceAFWeb.AccGetAccountStockTransList(accountObj.getId(), stock.getId(), trObj.getTrname(), 1);
...
//                            int ret = serviceAFWeb.SystemAddTransactionOrder(accountObj, stock, trObj.getTrname(), signal, dateOffet);
                    ...
                    trObj = serviceAFWeb.AccGetAccountStockIDByTRStockID(accountObj.getId(), stock.getId(), trObj.getTrname());
                    // need to get the latest TR object after the SystemAddTransactionOrder
...
//        this.serviceAFWeb = serviceAFWeb;
    ...
    public void updateAdminTradingsignal(ServiceAFweb serviceAFWeb, AccountObj accountObj, String symbol) {
        ...
        int size1year = 5 * 52;
        ArrayList StockArray = serviceAFWeb.InfGetStockHistorical(stock.getSymbol(), size1year * 4);
        if (StockArray == null) {
            return;
        }
        if (StockArray.size() < 10) {
...
//    }
    ...
    public void downloadNeuralNetWeight(ServiceAFweb serviceAFWeb, int nnNum) {
        ...
        if (nnNum == 0) {
            nnObj = serviceAFWeb.NnGetNeuralNetObjWeight0(serviceAFWeb, name, 0);
        } else if (nnNum == 1) {
            nnObj = serviceAFWeb.NnGetNeuralNetObjWeight1(serviceAFWeb, name, 0);
        }
        if (nnObj != null) {
    ...
    public AFneuralNet testNeuralNet0Symbol(ServiceAFweb serviceAFWeb, String nnName, String symbol) {
        ...
        AFneuralNet nnObj0 = serviceAFWeb.NnGetNeuralNetObjWeight0(serviceAFWeb, BPname, 0);
        if (nnObj0 == null) {
            return null;
        }
        return nnObj0;
...
//        nnObj1 = this.getCacheNNBP(serviceAFWeb, nnTraining);
        nnObj1 = serviceAFWeb.NnGetNeuralNetObjWeight0(serviceAFWeb, BPname, 0);
        if (nnObj1 == null) {
            return 0;
        }
    ...
    public int TrainingNNBP(ServiceAFweb serviceAFWeb, String nnNameSym, String nnNAme, NNTrainObj nnTraining, double nnError) {
        ...
        AFneuralNet afNeuralNet = serviceAFWeb.NnGetNeuralNetObjWeight1(serviceAFWeb, name, 0);
...
//                }
            ...
            serviceAFWeb.NnSetNeuralNetObjWeight1(serviceAFWeb, afNeuralNet);
        }
        if (nn == null) {
            nn = new NNBPservice();
...
//        }
        ...
        serviceAFWeb.NnSetNeuralNetObjWeight1(serviceAFWeb, afNeuralNet);
        serviceAFWeb.NnUpdateNeuralNetStatus1(name, ConstantKey.OPEN, afNeuralNet.getType());
...
//        }
        ...
        if (errorReturn < nnError) {
            ...
            if (nnError == 1) {
                ...
                double refminError = minError + 0.008; //+ 0.002;
                refData.setmError(refminError);

                serviceAFWeb.NnUpdateNeuralNetRef0(name, refData);
...
//                }
            ...
            if (getEnv.checkLocalPC() == true) {
                ...
                AFneuralNet nnObj0 = serviceAFWeb.NnGetNeuralNetObjWeight0(serviceAFWeb, name, 0);
                if (nnObj0 != null) {
        ...
        if (getEnv.checkLocalPC() == true) {
            ...
            AFneuralNet nnObj1 = serviceAFWeb.NnGetNeuralNetObjWeight1(serviceAFWeb, name, 0);
            if (nnObj1 != null) {
