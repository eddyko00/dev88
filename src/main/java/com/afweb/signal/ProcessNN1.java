                        ...
                        for (int j = 0; j < writeArray.size(); j++) {
                            ...
                            if (lastTH.getTrsignal() != nnSignal) {
                                ...
                                if (delta > 0) {
                                    ...
                                    if (delta > 0) {
                                        logger.info("> ProcessTRHistoryOffsetNN1 " + stock.getSymbol() + " Override 2 signal  " + stockDate.toString() + " date from last signal > 40 date");
                                        nnSignal = macdSignal;
                                    }
                                }
                                break; // for loop
...
//                }
        ...
        if (nnSignal != prevSignal) {
            ...
            if (nnSignal != trendSignal) {
                logger.info("> ProcessTRHistoryOffsetNN1 " + stock.getSymbol() + " Override 3 signal " + stockDate.toString() + " TrendSignal " + trendSignal);
            }
...
//                            MACDObj macdNN = TechnicalCal.MACD(StockArray, offset, ConstantKey.INT_MACD_12, ConstantKey.INT_MACD_26, ConstantKey.INT_MACD_9);
                        ...
                        if (thList != null) {
                            if (thList.size() > 0) {
                                ...
                                if (delta > 0) {
                                    ...
                                    delta = specialOverrideRule2(nn, lastTHLong, curSGLong);
                                    if (delta > 0) {
                                        logger.info("> updateAdminTradingsignalnn1 " + symbol + " Override 2 signal " + stockDate.toString() + " date from last signal > 40 date");
                                        nnSignal = macdSignal;
                                    }
                                }
                ...
                if (nnSignal != prevSignal) {
                    ...
                    if (nnSignal != trendSignal) {
                        logger.info("> updateAdminTradingsignalnn1 " + stock.getSymbol() + " Override 3 signal " + stockDate.toString() + " TrendSignal " + trendSignal);
                    }
                    nnSignal = trendSignal;
                }
                trObj.setTrsignal(nnSignal);
