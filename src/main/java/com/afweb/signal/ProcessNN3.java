...
public class ProcessNN3 {
    ...
    //StockArray assume recent date to old data
    //StockArray assume recent date to old data   
    public ArrayList<NNInputDataObj> trainingNN3dataMACD(ServiceAFweb serviceAFWeb, String nnName, String sym, ArrayList<AFstockInfo> StockArray, int offset, int monthSize) {
...
//                                    nnSignal = macdSignal;
    ...
    public float speciaOverridelRule1(float thClose, float StClose) {
    ...
    public float specialOverrideRule2(NNObj nn, long lastTHLong, long curSGLong) {
        ...
        if ((threshold > output1) && (threshold > output2)) {
            long next15date = TimeConvertion.addDays(lastTHLong, 20);  // check if last tran > 15 days
